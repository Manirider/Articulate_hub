...
const submodules = [
  {
    ...
    description: 'Watch AI-generated examples and learn optimal techniques before practicing.',
  ...
  },
  {
    ...
    description: 'Solo mode with full AI analysis. Practice at your own pace with detailed feedback.',
  ...
  },
  {
    ...
    description: 'Interactive sessions with AI avatars. Real-time conversation and coaching.',
  ...
  },
  {
    ...
    description: 'Multi-user sessions with AI observer. Practice with friends and get comparative analysis.',
      ...
      <main className="mx-auto max-w-5xl p-6 lg:p-8">
        ...
        <section className="glass rounded-2xl p-8 animate-fade-in-up">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-slate-100">{moduleName}</h1>
        ...
        <section className="mt-6">
          <h2 className="text-xl font-bold text-slate-100 mb-4">Choose Practice Mode</h2>
          <div className="grid gap-4 sm:grid-cols-2">
            {submodules.map((sub) => (
              <button
                key={sub.slug}
                onClick={() => router.push(`/modules/${params.module}/${sub.slug}`)}
                onMouseEnter={() => setHoveredMode(sub.slug)}
                onMouseLeave={() => setHoveredMode(null)}
                className={`glass card-hover rounded-2xl p-6 text-left ${sub.borderColor} ${sub.hoverBorder} transition-all duration-300`}
              >
                <div className="flex items-center gap-3">
                  <div className={`rounded-xl bg-gradient-to-br ${sub.color} p-3 text-white`}>
                    {sub.icon}
                  </div>
        ...
        <section className="mt-8 glass rounded-2xl p-8 flex flex-col items-center text-center animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
          ...
          <p className="mt-4 text-slate-400 text-sm max-w-md">
            {hoveredMode
              ? 'Ready to coach you! Select a mode to begin your practice session.'
              : 'Your AI coach is standing by. Hover over a practice mode to activate.'}
          </p>
        </section>
      </main>
    </div>
